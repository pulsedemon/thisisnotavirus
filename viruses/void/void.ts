...
class Void {
  ...
  createBeautyRings() {
    ...
    for (let i = 0; i < 5; i++) {
      const ringGeometry = new THREE.RingGeometry(70 + i * 25, 75 + i * 25, 6 + i * 2);
  ...
  createCorruptionTentacles() {
    ...
    for (let i = 0; i < 12; i++) { // More tentacles!
      const points = [];
  ...
  createTrailParticles() {
    ...
    // Clean up old trails
    if (this.trailParticles.length > 3) { // Reduced from 5 to 3
  ...
  autonomousRage() {
    ...
    if (this.rageBuildupLevel > 0.3 && Math.random() < 0.008) { // Reduced frequency
      // Spontaneous corruption burst
      this.createExplosion();
    }
    
    if (this.rageBuildupLevel > 0.5 && Math.random() < 0.003 && this.gravityWells.length < 2) { // Limit gravity wells
    ...
    if (this.rageBuildupLevel > 0.7 && Math.random() < 0.001 && !this.isHavingTantrum) { // Prevent multiple tantrums
  ...
  hungerForAttention() {
    ...
    if (this.rageBuildupLevel > 0.4) {
      ...
      this.evilEyes.forEach((eye, index) => {
        if (Math.random() < 0.1) {
    ...
    if (this.rageBuildupLevel > 0.6) {
      ...
      this.corruptionTentacles.forEach((tentacle, index) => {
        tentacle.userData.writheSpeed = 0.02 + index * 0.005 + this.rageBuildupLevel * 0.03;
      });
    }
  }
  ...
  updateParticles() {
    ...
    for (let i = 0; i < positions.length; i += 3) {
      ...
      positions[i + 2] = z + Math.sin(this.currentTime * 0.005 + i * 0.01) * 0.3;
    }
  ...
  render() {
    this.currentTime += this.timeWarp;
    
    // Gradually decay energy over time
    this.energyLevel *= 0.99;
    
    // Decay screech effect
    this.screechLevel *= 0.95;
    ...
    if (this.idleTime > 300) { // After 5 seconds of no interaction
      this.rageBuildupLevel = Math.min((this.idleTime - 300) / 1200, 1.0); // Maxes out after 20 seconds
    } else {
      this.rageBuildupLevel *= 0.98; // Quickly decay rage when interacting
    }
    ...
    const totalEnergy = this.energyLevel + audioLevel * 0.5 + this.rageBuildupLevel * 0.8;
    
    // Smooth mouse interaction with more responsiveness
    this.targetX += (this.mouseX - this.targetX) * (0.1 + totalEnergy * 0.2);
    this.targetY += (this.mouseY - this.targetY) * (0.1 + totalEnergy * 0.2);
    ...
    const ragePulse = this.isHavingTantrum ? Math.sin(this.currentTime * 0.2) * 0.5 : 0;
    const evilPulse = 1 + Math.sin(this.currentTime * (0.05 + this.rageBuildupLevel * 0.1)) * (0.3 + this.rageBuildupLevel * 0.4) + totalEnergy * 0.5 + ragePulse;
    this.flowingSphere.scale.setScalar(evilPulse);
    
    // Corrupt the color based on energy AND RAGE
    const coreMaterial = this.flowingSphere.material as THREE.MeshBasicMaterial;
    ...
    this.flowingSphere.children.forEach((child, index) => {
      ...
      if (index === 0) {
        ...
        childMaterial.opacity = 0.3 + Math.sin(this.currentTime * 0.08 + index) * 0.2 + totalEnergy * 0.4;
      }
    });
    ...
    this.beautyRings.children.forEach((ring, index) => {
      ...
      ring.rotation.y += 0.003 * this.timeWarp * Math.sin(this.currentTime * 0.01 + userData.corruptionPhase);
      
      // Evil corruption breathing effect
      const mouseInfluence = Math.abs(this.targetX) + Math.abs(this.targetY);
      const corruptionPulse = 1 + Math.sin(this.currentTime * 0.02 + userData.corruptionPhase) * (0.15 + mouseInfluence * 0.2 + totalEnergy * 0.4);
      ring.scale.setScalar(corruptionPulse);
      
      // Dynamic evil coloring
      const material = ring.material as THREE.MeshBasicMaterial;
      const corruptionIntensity = 0.3 + mouseInfluence * 0.4 + audioLevel * 0.6 + totalEnergy * 0.5;
    ...
    this.scene.children.forEach((child) => {
      if (child.userData && child.userData.energyResponse) {
        const material = child.material as THREE.MeshBasicMaterial;
        material.opacity = totalEnergy * child.userData.energyResponse;
        
        const scale = 1 + totalEnergy * 0.5 + Math.sin(this.currentTime * 0.01 + child.userData.phase) * 0.1;
    ...
    this.corruptionTentacles.forEach((tentacle, index) => {
      const userData = tentacle.userData;
      
      // INSANE multi-axis writhing motion
      tentacle.rotation.z += userData.writheSpeed * this.timeWarp * userData.thrashIntensity;
      tentacle.rotation.x += 0.02 * Math.sin(this.currentTime * 0.03 + userData.chaosPhase) * userData.thrashIntensity;
      tentacle.rotation.y += 0.015 * Math.cos(this.currentTime * 0.025 + index) * userData.thrashIntensity;
      
      // Corruption pulsing with more violence
      userData.corruptionLevel *= 0.97;
      const corruption = userData.corruptionLevel + totalEnergy * 0.7 + this.rageBuildupLevel * 0.8;
      
      // Dynamic morphing of the tentacle shape!
      if (Math.random() < 0.05 && corruption > 0.3) {
        // Randomly deform the tentacle geometry
        const geometry = tentacle.geometry as THREE.TubeGeometry;
        const positionAttribute = geometry.attributes.position;
        
        for (let i = 0; i < positionAttribute.count; i++) {
          const chaos = (Math.random() - 0.5) * corruption * 10;
          const x = positionAttribute.getX(i);
          const y = positionAttribute.getY(i);
          const z = positionAttribute.getZ(i);
      ...
      const violentScale = 1 + corruption * 0.5 + Math.sin(this.currentTime * 0.1 + userData.chaosPhase) * corruption * 0.3;
      tentacle.scale.setScalar(violentScale);
      
      // Tentacles lash out randomly when highly corrupted
      if (corruption > 0.6) {
        tentacle.position.x = Math.sin(this.currentTime * userData.writheSpeed + userData.chaosPhase) * corruption * 20;
        tentacle.position.y = Math.cos(this.currentTime * userData.writheSpeed * 1.3 + userData.chaosPhase) * corruption * 20;
        tentacle.position.z = Math.sin(this.currentTime * userData.writheSpeed * 0.7 + userData.chaosPhase) * corruption * 15;
      } else {
      ...
      if (this.rageBuildupLevel > 0.4 && Math.random() < 0.02) {
        const reachDirection = new THREE.Vector3(this.targetX * 100, this.targetY * 100, 0);
        reachDirection.normalize();
        tentacle.position.add(reachDirection.multiplyScalar(this.rageBuildupLevel * 10));
      }
    });

    // Animate evil eyes
    this.evilEyes.forEach((eye, index) => {
      const userData = eye.userData;
      
      // Eyes track the mouse like they're watching
      const lookDirection = new THREE.Vector3(this.targetX * 100, this.targetY * 100, 0);
      ...
      const intensity = userData.watchIntensity + totalEnergy * 0.3 + this.screechLevel * 0.5;
    ...
    // Update blood veins with proper cleanup
    ...
    this.camera.position.x = Math.sin(this.currentTime * 0.001) * 20 + this.targetX * cameraRadius;
    this.camera.position.y = Math.cos(this.currentTime * 0.0015) * 15 + this.targetY * 50;
